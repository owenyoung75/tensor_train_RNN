...
def tensor_rnn_with_feed_prev(cell, inputs, is_training, config, initial_states=None):
    ...
    with tf.variable_scope("trnn") as varscope:
        ...
        if initial_states is None:
            initial_states =[]
            for _ in range(config.num_lags):
                initial_state =  cell.zero_state(batch_size, dtype= tf.float32)
...
#def tensor_rnn_with_feed_prev(cell, inputs, is_training, config, initial_state=None):
#    """High Order Recurrent Neural Network Layer"""
#    outputs = []
#    prev = None
#    is_sample = is_training and initial_state is not None
#
#    with tf.variable_scope("trnn") as varscope:
#        if varscope.caching_device is None:
#                    varscope.set_caching_device(lambda op: op.device)
...
#                    #print("t", time_step, ">=", burn_in_steps, "--> feeding back output into input.")
#
#            print("ini state", initial_state)
#            expanded_state = tuple(map(lambda x:_poly_expand(x,3,13,batch_size),initial_state))
#            print("expanded_state", expanded_state)
#
#            """input tensor is [batch_size, num_steps, input_size]"""
#            """expanded_state is [batch_size, num_steps, total_state_size]"""
#            (cell_output, state)=cell(inp, expanded_state)
#
#            prev = cell_output
#            with tf.variable_scope(tf.get_variable_scope(), reuse=False):
#                output = fully_connected(cell_output, input_size, activation_fn=tf.sigmoid)
